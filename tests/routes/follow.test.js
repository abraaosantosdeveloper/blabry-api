...
const mockEstado = {
  idPorAlias: { outrousuario: OUTRO_ID, eu: USUARIO_ID },
  seguidores: 7,
  chamadas: [],
};
...
jest.mock('../../repositories/users_repository', () =>
  class UsuariosRepositoryFalso {
    ...
    async seguir(id, seguidorId, seguidoId) {
      mockEstado.chamadas.push({ acao: 'seguir', id, seguidorId, seguidoId });
    }
    async deixarDeSeguir(seguidorId, seguidoId) {
      mockEstado.chamadas.push({ acao: 'deixarDeSeguir', seguidorId, seguidoId });
    }
    async contarSeguidores() {
      return mockEstado.seguidores;
    }
    async buscarPerfil() { return null; }
    async buscar() { return { usuarios: [], total: 0 }; }
  }
);
...
describe('POST /users/:alias/follow', () => {
  ...
  it('segue e devolve o total recontado', async () => {
    ...
    expect(res.body).toEqual({ seguindo: true, seguidores: 7 });
  });
  ...
  it('usa o usuário do token como seguidor', async () => {
    ...
    expect(mockEstado.chamadas[0]).toMatchObject({
      acao: 'seguir',
      seguidorId: USUARIO_ID,
      seguidoId: OUTRO_ID,
    });
  });
  ...
  it('aceita o @ escrito com arroba na URL', async () => {
    ...
    expect(res.status).toBe(200);
    expect(mockEstado.chamadas[0].seguidoId).toBe(OUTRO_ID);
  });
  ...
  /* A restrição UNIQUE não pega este caso: o par seria válido no banco. */
  ...
  it('gera um identificador para a linha de relacionamento', async () => {
    await seguir('outrousuario');

    expect(mockEstado.chamadas[0].id).toEqual(expect.any(String));
    expect(mockEstado.chamadas[0].id.length).toBe(36);   // UUID
  });
});

describe('DELETE /users/:alias/follow', () => {
  it('deixa de seguir e devolve o total recontado', async () => {
    mockEstado.seguidores = 6;
    const res = await deixarDeSeguir('outrousuario');

    expect(res.status).toBe(200);
    expect(res.body).toEqual({ seguindo: false, seguidores: 6 });
  });
